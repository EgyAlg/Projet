...
class MyAttackStrategy(Strategy):
    ...
    def compute_strategy(self,state,id_team,id_player):
        ...
        p_cage2=Vector2D(GAME_WIDTH,GAME_GOAL_HEIGHT/2)
        p_cage1=Vector2D(0,GAME_GOAL_HEIGHT/2)
        ...
        if(id_team == 1):
            ...
            return my_state.shoot(p_cage2)
        else: 
            if((my_state.ball_position()-my_state.my_position()).norm>PLAYER_RADIUS+BALL_RADIUS):
                return my_state.aller(my_state.ball_position())
            return my_state.shoot(p_cage1)
...
class MyDefenseStrategy(Strategy):
    ...
    def compute_strategy(self,state,id_team,id_player):
        ...
        if(id_team == 1):
            if((my_state.ball_position()-p_cage1).norm<=30):
                if((my_state.ball_position()-my_state.my_position()).norm>PLAYER_RADIUS+BALL_RADIUS):
                    return my_state.aller(my_state.ball_position())    
                else:                    
                    return my_state.shoot(p_cage2)
        else: 
            if((my_state.ball_position()-p_cage2).norm<=30):
                if((my_state.ball_position()-my_state.my_position()).norm>PLAYER_RADIUS+BALL_RADIUS):
                    return my_state.aller(my_state.ball_position())    
                else:                    
                    return my_state.shoot(p_cage1)          
